...
/**
 * A YouTube player behind a poster.
 *
 * Nothing from YouTube is loaded until the visitor presses play. That is the
 ...
 * Until then the frame is the studio's own still, under the same grain as
 * every other frame on the site, with one play mark and nothing else.
 *
 * On play the iframe replaces the poster in place and autoplays, so a single
 * press starts the film rather than revealing a second play button. The
 * privacy-enhanced host is used so no tracking cookie is set by the embed.
 */
export function YouTubeEmbed({
  ...
  return (
    <div
      className={cn(
        'relative aspect-video overflow-hidden bg-[#0A131F]',
        className,
      )}
    >
      {isPlaying ? (
